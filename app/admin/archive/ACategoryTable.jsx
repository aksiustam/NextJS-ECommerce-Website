...
const ACategoryTable = (props) => {
  ...
  const onDelete = async (data) => {
    Swal.fire({
      title: data.name + " Adlı Kategori Silinecek!! ",
      showDenyButton: true,
      confirmButtonText: "Sil",
      denyButtonText: "Hayır",
    }).then(async (result) => {
      if (result.isConfirmed) {
        const res = await delArchiveCategory("category", data);
        if (res === true) {
          Swal.fire({
            icon: "success",
  ...
  const onSubmit = async (data) => {
    const res = await setArchiveCategory("category", data);
    if (res === true) {
      Swal.fire({
        icon: "success",
