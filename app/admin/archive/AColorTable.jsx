...
const AColorTable = (props) => {
  ...
  const onDelete = async (data) => {
    Swal.fire({
      title: data.name + " Adlı Renk Silinecek!! ",
      showDenyButton: true,
      confirmButtonText: "Sil",
      denyButtonText: "Hayır",
    }).then(async (result) => {
      if (result.isConfirmed) {
        const res = await delArchiveCategory("color", data);
        if (res === true) {
          Swal.fire({
            icon: "success",
  ...
  const onSubmit = async (data) => {
    const res = await setArchiveCategory("color", data);
    if (res === true) {
      Swal.fire({
        icon: "success",
