...
const ABrandTable = (props) => {
  ...
  const onDelete = async (data) => {
    Swal.fire({
      title: data.name + " Adlı Çeşit Silinecek!! ",
      showDenyButton: true,
      confirmButtonText: "Sil",
      denyButtonText: "Hayır",
    }).then(async (result) => {
      if (result.isConfirmed) {
        const res = await delArchiveCategory("brand", data);
        if (res === true) {
          Swal.fire({
            icon: "success",
  ...
  const onSubmit = async (data) => {
    const res = await setArchiveCategory("brand", data);
    if (res === true) {
      Swal.fire({
        icon: "success",
